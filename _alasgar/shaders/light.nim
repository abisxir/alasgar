...
proc sampleShadow(POSITION: Vec4,
  ...
  if DEPTH_MAP_LAYER >= 0:
    let
      ...
      shadowDepth = texture(DEPTH_MAPS, vec4(lightSpacePosition.xy,
          DEPTH_MAP_LAYER.float, lightSpacePosition.z))
    if shadowDepth < SHADOW_BIAS:
      result = shadowDepth

proc getLight*(LIGHT: Light, FRAGMENT: Fragment, SURFACE: Surface,
    DEPTH_MAPS: Sampler2DArrayShadow): Vec3 =
  var
    L: Vec3
    NoL: float
    intensity: float
  ...
  if intensity > 0.0:
    let
      shadow = sampleShadow(SURFACE.POSITION, LIGHT.SHADOW_MVP,
          LIGHT.SHADOW_BIAS, DEPTH_MAPS, LIGHT.DEPTH_MAP_LAYER)
