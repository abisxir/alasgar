...
proc newAnimationClipComponent*(animator: AnimatorComponent,
    name: string): AnimationClipComponent =
...
template `startTime`*[T: SomeValue](track: AnimationTrack[
    T]): float32 = track.frames[0].time
template `endTime`*[T: SomeValue](track: AnimationTrack[
    T]): float32 = track.frames[^1].time
template `duration`*[T: SomeValue](track: AnimationTrack[
    T]): float32 = track.endTime - track.startTime
template `size`*[T: SomeValue](track: AnimationTrack[T]): int = len(track.frames)
proc resize*[T: SomeValue](track: var AnimationTrack[T], size: int) = setLen(
    track.frames, size)
...
proc sampleConstant[T: SomeValue](track: AnimationTrack[T], thisFrame: int,
    t: float32): T =
  result = track.frames[thisFrame].value

proc sampleLinear[T: SomeValue](track: AnimationTrack[T], thisFrame: int,
    t: float32): T =
  let
...
proc sampleCubic[T: SomeValue](track: AnimationTrack[T], thisFrame: int,
    t: float32): T =
  let
    nextFrame = thisFrame + 1
    a = track.frames[thisFrame].time
...
func `interpolation`*[T: SomeValue](track: AnimationTrack[
    T]): InterpolationMode = track.interpolation
func `interpolation=`*[T: SomeValue](track: var AnimationTrack[T],
...
proc sample*[T: SomeValue](track: AnimationTrack[T], t: float32,
    valid: var bool): T =
...
method process*(sys: AnimationSystem, scene: Scene, input: Input,
    delta: float32, frames: int, age: float32) =
