...
proc doAmbientOcclusion(CAMERA: Camera, DEPTH_CHANNEL: Sampler2D, COORD: Vec2,
    P, N: Vec3, SCALE, MAX_DISTANCE, BIAS: float): float =
  let
...
proc ssao(CAMERA: Uniform[Camera],
  ...
  for i in 0..<SAMPLES:
    ...
    radius += rStep
    ao += doAmbientOcclusion(CAMERA, DEPTH_CHANNEL, UV + spiralUV * radius, p,
        n, SCALE, MAX_DISTANCE, BIAS)
