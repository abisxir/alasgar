...
proc isInSight*(viewCenter: Vec3, viewRadiusSqrt: float32,
...
proc packMaterial*(drawable: ptr Drawable) =
  ...
  if drawable.material == nil:
    ...
    if drawable.materialVersion != drawable.material.version:
      ...
      drawable.materialPack[0] = packUnorm4x8(material.diffuseColor.r,
          material.diffuseColor.g, material.diffuseColor.b,
          material.diffuseColor.a)
      drawable.materialPack[1] = packUnorm4x8(material.specularColor.r,
          material.specularColor.g, material.specularColor.b,
          material.uvChannels.float32 / 63.float32)
      drawable.materialPack[2] = packUnorm4x8(material.emissiveColor.r,
          material.emissiveColor.g, material.emissiveColor.b,
          material.availableMaps.float32 / 63.float32)
      drawable.materialPack[3] = packUnorm4x8(material.metallic,
          material.roughness, material.reflectance, material.ao)
      drawable.spritePack = vec4(material.frameSize.x, material.frameSize.y,
          material.frameOffset.x, material.frameOffset.y)
  ...
  if drawable.skin != nil:
    drawable.skinPack = vec4(drawable.skin.count.float32,
        drawable.skin.offset.float32, 0, 0)
  else:
    drawable.skinPack = vec4(0, 0, 0, 0)

method process*(sys: PrepareSystem, scene: Scene, input: Input, delta: float32,
    frames: int, age: float32) =
  if scene.root != nil:
    ...
    for drawable in mitems(scene.drawables):
      drawable.visible =
        drawable.transform.entity.visible and
            drawable.transform.entity.attached and
            drawable.transform.entity.opacity > 0
