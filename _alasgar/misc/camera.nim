...
proc handleInput(component: InputComponent, input: Input) =
  ...
  if getMouseButtonDown(input, mouseButtonLeft):
    ...
    controller.distance -= input.mouse.wheel.y * runtime.delta *
        controller.speed * 2

  controller.delta = vec2(clamp(mouseDelta.x, -MAX_STEP, MAX_STEP), clamp(
      mouseDelta.y, -MAX_STEP, MAX_STEP))
...
proc addCameraController*(e: Entity, distance = 10.0, theta = PI/4.0,
    phi = PI/4.0) =
