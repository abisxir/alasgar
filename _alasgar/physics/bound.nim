...
method intersects*(box: BoxBound, ray: Ray): Collision =
  ...
  if isRayIntersectsBox(ray.origin, ray.normalizedDirection,
      ray.inversedNormalizedDirection, ray.length, min(box), max(box), addr(
      intersectPoint)):
...
method intersects*(sphere: SphereBound, bound: Bound): Collision =
  if bound of SphereBound:
    var other = cast[SphereBound](bound)
    if isSphereIntersectsSphere(sphere.center, sphere.radius * sphere.scale.x,
        other.center, other.radius):
      result = newCollision()
  elif bound of BoxBound:
    var other = cast[BoxBound](bound)
    if isSphereIntersectsBox(sphere.center, sphere.radius * sphere.scale.x, min(
        other), max(other)):
      result = newCollision()
