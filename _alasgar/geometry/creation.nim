...
proc rotate*(shape: var Shape, axis: Vec3) =
  ...
  for i in 0..<len(shape.vertices):
    echo shape.vertices[i].position, " * ", rotation, " = ", shape.vertices[
        i].position * rotation
...
proc box*(extent: Extent, uvOffset = vec2(0.0), uvScale = vec2(1.0, 1.0)): Shape =
  ...
  add(result.vertices, [
     newVertex(position = vec3(-width / 2, -height / 2, depth / 2),
         uv0 = uvOffset + vec2(0, 0) * uvScale, normal = vec3(0, 0, 1)),
     newVertex(position = vec3(width / 2, -height / 2, depth / 2),
         uv0 = uvOffset + vec2(1, 0) * uvScale, normal = vec3(0, 0, 1)),
     newVertex(position = vec3(width / 2, height / 2, depth / 2),
         uv0 = uvOffset + vec2(1, 1) * uvScale, normal = vec3(0, 0, 1)),
     newVertex(position = vec3(-width / 2, height / 2, depth / 2),
         uv0 = uvOffset + vec2(0, 1) * uvScale, normal = vec3(0, 0, 1)),
     # Back
    newVertex(position = vec3(-width / 2, -height / 2, -depth / 2),
        uv0 = uvOffset + vec2(1, 0) * uvScale, normal = vec3(0, 0, -1)),
    newVertex(position = vec3(-width / 2, height / 2, -depth / 2),
    ...
    # Top
    newVertex(position = vec3(-width / 2, height / 2, -depth / 2),
        uv0 = uvOffset + vec2(0, 1) * uvScale, normal = vec3(0, 1, 0)),
    newVertex(position = vec3(-width / 2, height / 2, depth / 2),
        uv0 = uvOffset + vec2(0, 0) * uvScale, normal = vec3(0, 1, 0)),
    newVertex(position = vec3(width / 2, height / 2, depth / 2),
        uv0 = uvOffset + vec2(1, 0) * uvScale, normal = vec3(0, 1, 0)),
    newVertex(position = vec3(width / 2, height / 2, -depth / 2),
        uv0 = uvOffset + vec2(1, 1) * uvScale, normal = vec3(0, 1, 0)),
    # Bottom
    newVertex(position = vec3(-width / 2, -height / 2, -depth / 2),
        uv0 = uvOffset + vec2(0, 0) * uvScale, normal = vec3(0, -1, 0)),
    newVertex(position = vec3(width / 2, -height / 2, -depth / 2),
    ...
    # Right
    newVertex(position = vec3(width / 2, -height / 2, -depth / 2),
        uv0 = uvOffset + vec2(1, 0) * uvScale, normal = vec3(1, 0, 0)),
    newVertex(position = vec3(width / 2, height / 2, -depth / 2),
        uv0 = uvOffset + vec2(1, 1) * uvScale, normal = vec3(1, 0, 0)),
    newVertex(position = vec3(width / 2, height / 2, depth / 2),
        uv0 = uvOffset + vec2(0, 1) * uvScale, normal = vec3(1, 0, 0)),
    newVertex(position = vec3(width / 2, -height / 2, depth / 2),
        uv0 = uvOffset + vec2(0, 0) * uvScale, normal = vec3(1, 0, 0)),
    # Left
    newVertex(position = vec3(-width / 2, -height / 2, -depth / 2),
        uv0 = uvOffset + vec2(0, 0) * uvScale, normal = vec3(-1, 0, 0)),
    newVertex(position = vec3(-width / 2, -height / 2, depth / 2),
...
proc plane*(extent: Extent, uvOffset = vec2(0.0), uvScale = vec2(1.0, 1.0)): Shape =
  add(result.vertices, [
      newVertex(position = vec3(-0.5, 0.0, 0.5), normal = VEC3_UP,
          uv0 = uvOffset + vec2(0, 0) * uvScale),
      newVertex(position = vec3(-0.5, 0, -0.5), normal = VEC3_UP,
          uv0 = uvOffset + vec2(0, 1) * uvScale),
      newVertex(position = vec3(0.5, 0, -0.5), normal = VEC3_UP,
          uv0 = uvOffset + vec2(1, 1) * uvScale),
      newVertex(position = vec3(0.5, 0, 0.5), normal = VEC3_UP, uv0 = uvOffset +
          vec2(1, 0) * uvScale),
  ])
  add(result.indices, [
      0.uint32, 1, 2,
