...
func step*(e1, e2: Vec3): Vec3 = vec3(step(e1.x, e2.x), step(e1.y, e2.y), step(
    e1.z, e2.z))
func step*(e1, e2: Vec4): Vec4 = vec4(step(e1.x, e2.x), step(e1.y, e2.y), step(
    e1.z, e2.z), step(e1.w, e2.w))
...
proc min*(v: Vec4, f: float32): Vec4 = vec4(min(v.x, f), min(v.y, f), min(v.z,
    f), min(v.w, f))
proc max*(v: Vec2, f: float32): Vec2 = vec2(max(v.x, f), max(v.y, f))
proc max*(v: Vec3, f: float32): Vec3 = vec3(max(v.x, f), max(v.y, f), max(v.z, f))
proc max*(v: Vec4, f: float32): Vec4 = vec4(max(v.x, f), max(v.y, f), max(v.z,
    f), max(v.w, f))
...
proc inversesqrt*(x: Vec3): Vec3 = vec3(inversesqrt(x.x), inversesqrt(x.y),
    inversesqrt(x.z))
proc inversesqrt*(x: Vec4): Vec4 = vec4(inversesqrt(x.x), inversesqrt(x.y),
    inversesqrt(x.z), inversesqrt(x.w))
...
proc pow*(a, b: Vec4): Vec4 = vec4(pow(a.x, b.x), pow(a.y, b.y), pow(a.z, b.z),
    pow(a.w, b.w))
