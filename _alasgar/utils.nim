...
export logger, helpers, strformat, chroma, strutils, sequtils, opengl, error,
    helpers, vector, matrix, types, quat, sugar

# Extracts data out of the given pointer
template offsetOf*[T](p: ptr T, o: int): T = cast[ptr T](cast[ByteAddress](p) +
    (o * sizeof(T)))
...
func `bytes`*(c: Color): (uint8, uint8, uint8, uint8) =
  result = ((c.r * 255.0).uint8, (c.g * 255.0).uint8, (c.b * 255.0).uint8, (
      c.a * 255.0).uint8)
