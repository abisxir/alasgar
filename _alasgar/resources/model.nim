...
proc addMesh*(r: ModelResource, nodeName: string, vertices: var seq[
    Vertex]): Mesh = addMesh(r, nodeName, newMesh(vertices))
...
proc hasMaterial*(r: ModelResource, nodeName: string): bool = hasKey(
    r.materials, nodeName)
proc addMaterial*(r: ModelResource, nodeName: string,
    material: MaterialComponent): MaterialComponent =
...
proc addJoint*(r: ModelResource, nodeName: string, skin: string,
    inverseMatrix: Mat4) = add(r.joints, (nodeName, skin, inverseMatrix))

proc hasAnimationClip*(r: ModelResource, nodeName: string): bool = hasKey(
    r.clips, nodeName)
...
proc hasAnimationChannel*(r: ModelResource, nodeName: string): bool = hasKey(
    r.channels, nodeName)
proc addAnimationChannel*(r: ModelResource, nodeName: string, clipName: string,
...
proc addNode*(r: ModelResource, name, parent: string, position: Vec3,
    rotation: Quat, scale: Vec3): ModelNode =
...
proc toEntity*(r: Resource, scene: Scene, castShadow = false,
    rootName = ""): Entity =
  var
