...
proc getFilename(fullpath: string): string = filterIt(split(fullpath, "/"),
    not isEmptyOrWhitespace(it))[^1]
proc makeFileName(fullpath, filename: string): string = replace(fullpath,
...
proc loadMaterials(modelFileName: string, materialFileName: string,
    mr: ModelResource) =
...
proc loadObj(filename: string): Resource =
  ...
  while fileStream.readLine(buf):
    if startsWith(buf, "mtllib "):
      ...
      let node = addNode(modelResource, name = modelName, parent = "",
          position = VEC3_ZERO, rotation = quat(), scale = VEC3_ONE)
      node.mesh = modelName
    else:
