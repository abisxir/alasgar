...
proc newOrthoCamera*(left, right, bottom, top, near, far: float32,
    direction: Vec3, up: Vec3 = VEC3_UP): CameraComponent =
  new(result)
  result.left = left
  result.right = right
...
proc screenToWorldCoord*(pos: Vec2, windowSize: Vec2,
    camera: CameraComponent): Vec4 =
...
proc calculateViewCenter*(camera: CameraComponent, vOut: var Vec3,
    rOut: var float32) =
  vOut = camera.transform.globalPosition + (normalize(camera.direction) * ((
      camera.far - camera.near) / 2 + camera.near))
...
proc updateShader(
  ...
  shader["CAMERA.INVERSE_VIEW_PROJECTION_MATRIX"] = inverse(active.view *
      active.projection)
...
method process*(sys: CameraSystem, scene: Scene, input: Input, delta: float32,
  ...
  let mouseZW = if getMouseButtonDown(input,
      mouseButtonLeft): mouseXY else: mouseXY * -1
