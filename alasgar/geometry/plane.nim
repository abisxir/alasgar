...
proc generatePlaneMesh(rows, cols: uint32, vertices: var seq[Vertex],
    indices: var seq[uint32]) =
  for i in 0..rows:
    for j in 0..cols:
      var v: Vertex
      v.position = vec3(
...
proc plane1x1(): Mesh =
  var
    vertices = [
        newVertex(position = vec3(-0.5, 0.0, 0.5), normal = VEC3_UP, uv0 = vec2(
            0, 0)),
        newVertex(position = vec3(-0.5, 0, -0.5), normal = VEC3_UP, uv0 = vec2(
            0, 1)),
        newVertex(position = vec3(0.5, 0, -0.5), normal = VEC3_UP, uv0 = vec2(1,
            1)),
        newVertex(position = vec3(0.5, 0, 0.5), normal = VEC3_UP, uv0 = vec2(1,
            0)),
    ]
