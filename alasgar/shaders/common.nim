...
proc constructWorldNormal*(CAMERA: Camera, UV: Vec2,
    DEPTH_CHANNEL: Sampler2D): Vec3 =
  var
...
proc linearizeDepth*(CAMERA: Camera, UV: Vec2,
    DEPTH_CHANNEL: Sampler2D): float32 =
  let depth = texture(DEPTH_CHANNEL, UV).x
  return (2.0 * CAMERA.NEAR) / (CAMERA.FAR + CAMERA.NEAR - depth * (CAMERA.FAR -
      CAMERA.NEAR));
...
proc reconstructNormal*(CAMERA: Camera, UV: Vec2,
    DEPTH_CHANNEL: Sampler2D): Vec3 =
  let
...
proc isPBR*(FRAGMENT: Fragment): bool = FRAGMENT.ROUGHNESS > 0.0 or
