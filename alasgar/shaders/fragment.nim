...
proc prepareFragment*(SURFACE: Surface,
  ...
  result.FOG_AMOUNT = getFogAmount(ENVIRONMENT.FOG_DENSITY,
      ENVIRONMENT.FOG_MIN_DISTANCE, length(
      SURFACE.POSITION_RELATED_TO_VIEW.xyz))
  if result.FOG_AMOUNT < 1.0:
    ...
    if result.OPACITY > OPACITY_CUTOFF:
      ...
      if isPBR(result):
        ...
        result.F0 = mix(vec3(0.04 * result.REFLECTANCE * result.REFLECTANCE),
            result.ALBEDO, result.METALLIC)
        ...
        result.F0 = mix(vec3(0.04 * result.REFLECTANCE * result.REFLECTANCE),
            result.ALBEDO, 1.0 - result.REFLECTANCE)
