...
proc mainFragment*(SKIN_MAP: Layout[0, Uniform[Sampler2D]],
  ...
  if FRAGMENT.FOG_AMOUNT < 1.0:
    ...
    if FRAGMENT.OPACITY <= OPACITY_CUTOFF:
      ...
      if ENVIRONMENT.HAS_ENV_MAP > 0:
        COLOR.rgb = COLOR.rgb + ENVIRONMENT.INTENSITY * getIBL(ENVIRONMENT,
            FRAGMENT, SKYBOX_MAP)
  ...
  when defined(DEBUG_Z):
    let
      z = abs(SURFACE.POSITION_RELATED_TO_VIEW.z /
          SURFACE.POSITION_RELATED_TO_VIEW.w)
      c = CAMERA.FAR - CAMERA.NEAR
    COLOR.rgb = vec3(z / c)
    COLOR.a = 1.0
