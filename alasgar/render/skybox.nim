...
proc newSkybox*(): Skybox =
  ...
  glBufferData(GL_ARRAY_BUFFER, (sizeof(float32) * vertices.len).GLsizeiptr,
      addr(vertices[0]), GL_STATIC_DRAW)
  glVertexAttribPointer(0, 3, cGL_FLOAT, false, (3 * sizeof(float32)).GLsizei,
      cast[pointer](0))
...
proc render*(skybox: Skybox, cubemap: Texture, view, projection: Mat4,
    intensity, blurrity: float32) =
