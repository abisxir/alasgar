...
proc vs(POSITION: Vec3, UV: Vec2, CAMERA: Uniform[Camera], GRID_SCALE: Uniform[
    int], DATA: var Vec2, gl_Position: var Vec4) =
  var
...
proc fs(CAMERA: Uniform[Camera], TICKNESS: Uniform[float], GRID_COLOR: Uniform[
    Vec3], DATA: Vec2, COLOR: var Vec4) =
  COLOR.a = 0.0
...
proc newGrid*(scene: Scene, gridSize: int = 1, tickness: float = 0.02,
    color = rgb(0.5, 0.5, 0.5)): Entity =
  let
