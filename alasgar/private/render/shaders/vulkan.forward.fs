...
float v_smith_ggx_correlated(float roughness2, float NoV, float NoL, float lambdaVC) {
    float lambdaV = NoL * lambdaVC;
    float lambdaL = NoV * sqrt((NoL - roughness2 * NoL) * NoL + roughness2);
...
vec3 brdf(vec3 L, vec3 V, vec3 N, vec3 albedo, vec3 f0, float roughness, float roughness2, float NoV, float NoL, float lambdaVC) {
    vec3 H = normalize(V + L);

    float NoH = clamp(dot(N, H), 0.0, 1.0);
    float LoH = clamp(dot(L, H), 0.0, 1.0);

    float D = d_ggx(roughness2, NoH, N, H);
    vec3  F = f_schlick(f0, LoH);
    float G = v_smith_ggx_correlated(NoV, NoL, roughness2, lambdaVC);
...
void main() {
    ...
    float lambdaVC = sqrt((NoV - roughness2 * NoV) * NoV + roughness2);

    vec3 albedo = (1.0 - metallic) * material.base_color.rgb;
    ...
    for(int i = 0; i < point_light_count; i++) {
        vec3 light_to_surface = point_lights[0].position - surface.position.xyz;
        vec3 L = normalize(light_to_surface);
        ...
        vec3 effect = brdf(L, V, N, albedo, f0, roughness, roughness2, NoV, NoL, lambdaVC);
        ...
        lights_effect += point_lights[i].color * effect * illuminance * NoL;
    }
