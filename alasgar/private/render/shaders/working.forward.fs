...
vec3 calculate_specular_reflection(vec3 specularEnvR0, vec3 specularEnvR90, float VdH) {
    return specularEnvR0 + (specularEnvR90 - specularEnvR0) * pow5(clamp(1.0 - VdH, 0.0, 1.0));
}

float calculate_geometric_occlusion(float NdL, float NdV, float r2, float attenuationV) {
    float attenuationL = 2.0 * NdL / (NdL + sqrt(r2 + (1.0 - r2) * (NdL * NdL)));
    // We calculate this for all of lights.
    //float attenuationV = 2.0 * NdV / (NdV + sqrt(r * r + (1.0 - r * r) * (NdV * NdV)));
    return attenuationL * attenuationV;
}

float calculate_microfacet_distribution(float roughness2, float NdH) {
    float f = (NdH * roughness2 - NdH) * NdH + 1.0;
...
void main() {
    ...
    float attenuationV = 2.0 * NdV / (NdV + sqrt(roughness2 + (1.0 - roughness2) * (NdV * NdV)));

    vec3 Lo = vec3(0.0);
    int point_light_count = min(MAX_POINT_LIGHTS, env.point_lights_count);
    for(int i = 0; i < point_light_count; i++) {
        vec3 light_direction = point_lights[0].position - surface.position.xyz;
        ...
        vec3 F = calculate_specular_reflection(specularEnvR0, specularEnvR90, VdH);
        float G = calculate_geometric_occlusion(NdL, NdV, roughness2, attenuationV);
        float D = calculate_microfacet_distribution(roughness2, NdH);
        vec3 diffuseContrib = (1.0 - F) * (diffuseColor / PI);
        vec3 specContrib = F * G * D / (4.0 * NdL * NdV);
        
        // Shading based off lights
        Lo += NdL * point_lights[0].color * (diffuseContrib + specContrib);
