...
void main() {
    ...
    vec3 color = env.ambient_color;
    for(int i = 0; i < point_light_count; i++) {
        float distance = length(point_lights[i].position - surface.position.xyz);
        vec3 L = normalize(point_lights[i].position - surface.position.xyz);
        ...
        color += NdL * point_lights[i].color * (diffuse_contribute + specular_contribute);
    ...
    */
    ...
    // Adds emmisive color
    color += material.emmisive_color.rgb;
