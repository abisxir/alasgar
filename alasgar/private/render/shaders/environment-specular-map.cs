...
//1.0 / float(NumSamples);
const float InvNumSamples = 0.0009765625;

// In OpenGL only a single mip level is bound.
const int NumMipLevels = 1;
layout(binding=0) uniform samplerCube inputTexture;
layout(binding=0, rgba16f) writeonly uniform imageCube outputTexture[NumMipLevels];
...
vec3 getSamplingVector()
{
    ...
    vec2 st = vec2(x, y)/vec2(imageSize(outputTexture[PARAM_LEVEL]));
...
void main(void)
{
	...
	if(int(gl_GlobalInvocationID.x) >= outputSize.x || int(gl_GlobalInvocationID.y) >= outputSize.y) {
		return;
	}
	...
	for(int i=0; i<NumSamples; ++i) {
		vec2 u = sampleHammersley(uint(i));
