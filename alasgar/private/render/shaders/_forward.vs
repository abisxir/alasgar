...
uniform mat4 u_shadow_projection_matrux;

// Model
out vec2 v_uv;
out vec3 v_normal;
out vec4 v_fragment_position;
out vec4 v_position_related_to_view;
...
void main() {
    ...
    v_shadow_matrix = BIAS_MATRIX * u_shadow_mvp * inverse(u_view_matrix);
    ...
    v_position_related_to_view = u_view_matrix * v_fragment_position;
    if(u_fog_enabled > 0) {
        float distance = length(v_position_related_to_view);
    ...
    gl_Position = u_projection_matrix * v_position_related_to_view;
}
