...
vec3 BRDF_lambertian(vec3 f0, vec3 diffuseColor, float specularWeight, float VoH)
{
    vec3 f90 = vec3(1.0);
    vec3 F = f0 + (f90 - f0) * pow5(clamp(1.0 - VoH, 0.0, 1.0));
...
float V_GGX(float NdotL, float NdotV, float alphaRoughness)
{
    ...
    // TODO: could calculate this factor just once: sqrt(NdotV * NdotV * (1.0 - alphaRoughnessSq) + alphaRoughnessSq)
...
vec3 BRDF_specularGGX(vec3 f0, float alphaRoughness, float specularWeight, float VoH, float NoL, float NoV, float NoH, float ggx_factor_const)
{
    vec3 f90 = vec3(1.0);
    // TODO: make function
    vec3 F = f0 + (f90 - f0) * pow5(clamp(1.0 - VoH, 0.0, 1.0));
    float Vis = V_GGX(NoL, NoV, alphaRoughness);
...
void main() {
    ...
    float indirect_intensity = 0.5;
    float occlusion_strength = 1.0;
    float specular_weight = 0.5;

    vec3 f_specular = vec3(0.0);
    // TODO: use ambient color env.ambient_color
    vec3 f_diffuse = vec3(0.0);

    // Calculates indirect lights

    /*
    f_specular += get_ibl_radiance_ggx(N, V, NoV, roughness, f0, 1.0);
    f_diffuse += get_ibl_radiance_lambertian(N, V, NoV, roughness, albedo, f0, 1.0);
    */
    ...
    for(int i = 0; i < lights_count; i++) {
        ...
        f_diffuse += intensity * NoL * BRDF_lambertian(f0, albedo, specular_weight, VoH);
		f_specular += intensity * NoL * BRDF_specularGGX(f0, r2, specular_weight, VoH, NoL, NoV, NoH, ggx_factor_const);
    }
    ...
    out_color.rgb = linearTosRGB(f_emissive + f_diffuse + f_specular);
