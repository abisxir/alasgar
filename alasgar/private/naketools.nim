...
proc nimbleAlasgarPath(): string =
    result = nimblePath("alasgar")
    doAssert(result.len != 0, "Error: alasgar does not seem to be installed with nimble!")
...
proc setBuilderSettingsFromCmdLine(b: Builder) =
    ...
    echo " - Screen orientaion is: ", b.screenOrientation
...
proc iOSSDKPath*(version: string): string =
    result = xCodeApp/"Contents/Developer/Platforms/iPhoneOS.platform/Developer/SDKs/iPhoneOS" &
            version & ".sdk"
proc iOSSimulatorSDKPath*(version: string): string =
    result = xCodeApp/"Contents/Developer/Platforms/iPhoneSimulator.platform/Developer/SDKs/iPhoneSimulator" &
            version & ".sdk"
...
proc findEnvPaths(b: Builder) =
    if b.platform in ["android", "ios", "ios-sim"]:
        ...
        if b.platform == "android":
            ...
            if sdkPath.len == 0: sdkPath = getEnv(
                    "ANDROID_SDK_HOME").expandTilde()
            when not defined(windows):
                if sdkPath.len == 0:
                    sdkPath = "~/Library/Android/sdk"
                    if not fileExists(expandTilde(sdk_path /
                            "platform-tools/adb")):
                        sdkPath = ""
            if sdkPath.len == 0:
                ...
                var ndk = getEnv("ANDROID_NDK_HOME").expandTilde()
                if ndk.len == 0:
                    ndk = sdkPath / "ndk"
                    if dirExists(ndk):
                        ...
                        ndk = sdkPath / "ndk-bundle"
                        if dirExists(ndk):
                            b.androidNdk = ndk
                        else:
                            errorMsg &= "\nAndroid NDK (ndk or ndk-bundle) not installed in ANDROID_HOME (" &
                                sdkPath & "). To install it run: " & sdkPath & "/tools/bin/sdkmanager ndk-bundle"
                else:
                    b.androidNdk = ndk

            b.androidSdk = sdkPath

        #b.sdlRoot = nimbleAlasgarPath() / "assets" / "SDL"
        #b.sdlMixerRoot = nimbleAlasgarPath() / "assets" / "SDL_mixer"

        b.sdlRoot = getEnv("SDL_HOME")
        if b.sdlRoot.len == 0: 
            errorMsg &= getEnvErrorMsg("SDL_HOME")

        b.sdlMixerRoot = getEnv("SDL_MIXER_HOME")
        if b.sdlRoot.len == 0: 
            errorMsg &= getEnvErrorMsg("SDL_MIXER_HOME")
...
proc newBuilder*(platform: string): Builder =
    ...
    b.bundleId = "com.alasgar.Alasgar3D"
    b.javaPackageId = "com.alasgar.Alasgar3D"
    b.activityClassName = "com.alasgar.AlasgarActivity"
    ...
    if b.platform in ["ios", "ios-sim"]:
        ...
        var macosxSDK = execProcess("xcrun", args = ["--show-sdk-path"],
                options = {poUsePath})
        macosxSDK.removeSuffix()
        b.macOSSDKPath = macosxSDK
        var ver = execProcess("xcrun", args = ["--show-sdk-version"],
                options = {poUsePath})
        ver.removeSuffix()
...
proc convertResource*(b: Builder, origPath, destExtension: string, conv: proc(
...
proc runAppInSimulator(b: Builder) =
    ...
    direShell "xcrun", "simctl", "install", b.iOSSimulatorDeviceId,
            b.buildRoot / b.bundleName
    direShell "xcrun", "simctl", "launch", waitForDebugger,
            b.iOSSimulatorDeviceId, b.bundleId
...
proc buildSDLForDesktop(b: Builder): string =
    when defined(linux):
        ...
        direShell "xcodebuild", "-project", xcodeProjDir/"SDL.xcodeproj",
                "-target", "Static\\ Library", "-configuration", "Release",
                "-sdk", "macosx"&b.macOSSDKVersion, "SYMROOT=build"
        return result
    else:
...
proc checkSdlMixerRoot(b: Builder) =
    let r = expandTilde(b.sdlMixerRoot)
    if not dirExists(r / "src"):
        echo "Wrong SDL_MIXER_HOME. The SDL_MIXER_HOME environment variable must point to SDL2 mixer source code."
        raise newException(Exception, "Wrong SDL_MIXER_HOME")
...
proc buildSDLForIOS(b: Builder, forSimulator: bool = false): string =
    ...
    if not fileExists result/"libSDL2.a":
        var args = @["xcodebuild", "-project", xcodeProjDir/"SDL.xcodeproj",
                "-configuration", "Release", "-sdk", entity&b.iOSSDKVersion, "SYMROOT=build"]
        if forSimulator:
...
proc prepareSDLforAndroid(b: Builder, buildDir: string) =
    b.checkSdlRoot()
    let sdlRoot = expandTilde(b.sdlRoot)
    let alasgarTemplateDir = nimbleAlasgarPath() / "assets" / "android/template"
    let sdlDefaultAndroidProjectTemplate = sdlRoot/"android-project"

    createDir(buildDir)
    copyDirWithPermissions(alasgarTemplateDir, buildDir)
    copyDir(sdlDefaultAndroidProjectTemplate / "app/src/main/java",
...
proc prepareSDLMixerforAndroid(b: Builder, buildDir: string) =
    b.checkSdlMixerRoot()
    let sdlMixerRoot = expandTilde(b.sdlMixerRoot)

    let sdlMixerJni = buildDir/"jni"/"SDL_mixer"
    createDir(sdlMixerJni)

    copyDir sdlMixerRoot/"src", sdlMixerJni/"src"
    copyDir sdlMixerRoot/"include", sdlMixerJni/"include"
    #copyDir sdlMixerRoot/"external", sdlMixerJni/"external"

    let mk = sdlMixerJni/"Android.mk"
    copyFile(sdlMixerRoot/"Android.mk", mk)

    var mkData = readFile(mk)
    block cutSDLDLLBuild:
        # Patch SDL's Android.mk so that it doesn't build dynamic lib.
        let startIndex = mkData.find("include $(BUILD_SHARED_LIBRARY)")
        let endIndex = "include $(BUILD_SHARED_LIBRARY)".len
        mkData.delete(startIndex, startIndex + endIndex)

        mkData = replace(mkData, "SUPPORT_FLAC ?= true", "SUPPORT_FLAC ?= false")
        mkData = replace(mkData, "SUPPORT_OGG ?= true", "SUPPORT_OGG ?= false")
        mkData = replace(mkData, "SUPPORT_MP3_MPG123 ?= true", "SUPPORT_MP3_MPG123 ?= false")
        mkData = replace(mkData, "SUPPORT_MOD_MODPLUG ?= true", "SUPPORT_MOD_MODPLUG ?= false")
        mkData = replace(mkData, "SUPPORT_MID_TIMIDITY ?= true", "SUPPORT_MID_TIMIDITY ?= false")
...
proc makeAndroidBuildDir(b: Builder): string =
    ...
    if not dirExists buildDir:
        prepareSDLforAndroid(b, buildDir)
        prepareSDLMixerforAndroid(b, buildDir)
        ...
        var permissions = ""
        for p in b.androidPermissions: permissions &=
                "<uses-permission android:name=\"android.permission." & p & "\"/>\L"
        var debuggable = ""
        if b.debugMode:
        ...
        var screenOrientation = "android:screenOrientation=\"" &
            b.screenOrientation & "\""

        let vars = {
            "PACKAGE_ID": b.javaPackageId,
            "APP_NAME": b.appName,
            "APP_NAME_AS_STRING": &"'{b.appName}'",
        ...
        replaceVarsInFile buildDir/"settings.gradle", vars
...
proc postprocessWebTarget(b: Builder) =
    if not b.disableClosureCompiler and fileExists(b.executablePath):
        if b.platform == "js":
            closure_compiler.compileFileAndRewrite(b.executablePath,
                    ADVANCED_OPTIMIZATIONS, b.enableClosureCompilerSourceMap)
        # If source map is disabled, emscripten has already run build-in closure compiler
            ...
            closure_compiler.compileFileAndRewrite(b.executablePath,
                    SIMPLE_OPTIMIZATIONS, b.enableClosureCompilerSourceMap)

    let sf = splitFile(b.mainFile)
    var mainHTML = sf.dir / sf.name & ".html"
    if not fileExists(mainHTML):
        mainHTML = nimbleAlasgarPath() / "assets" / "main.html"
...
proc signIosBundle(b: Builder) =
    ...
    direShell(["codesign", "-s", "\"" & b.codesignIdentity & "\"", "--force",
            "--entitlements", entPath, b.buildRoot / b.bundleName])
...
proc makeEmscriptenPreloadData(b: Builder): string =
    ...
    var args = @["python", packagerPy.quoteShell(), b.buildRoot / "main.data",
            "--js-output=" & result]
        ...
        if dirExists(w64):
            ...
            if not dirExists(result):
                ...
                raise newException(Exception,
                        "No NDK toolchain found for windows in " & result)
    elif defined(macosx):
...
proc build*(b: Builder) =
    ...
    of "macosx":
        ...
        addCAndLFlags(["-isysroot", macOSSDK, "-mmacosx-version-min=" &
                b.macOSMinVersion])
    ...
    of "ios", "ios-sim":
        ...
        b.nimFlags.add(["--os:macosx", "-d:ios", "-d:iPhone",
                "--dynlibOverride:SDL2"])
    ...
    of "android":
        ...
        b.nimFlags.add(["--os:linux", "-d:android", "--cc:clang",
                "--dynlibOverride:SDL2"])
        ...
        when defined(macosx) or defined(linux):
            ...
            if mxeBin.len == 0:
                ...
                if mxeBin.len == 0:
                    ...
                    quit 1
            b.nimFlags.add(["--cpu:i386", "--os:windows", "--cc:gcc",
                    "--gcc.exe:" & mxeBin, "--gcc.linkerexe:" & mxeBin])
    ...
    of "emscripten", "wasm":
        ...
        b.nimFlags.add(["--cpu:i386", "-d:emscripten", "--os:linux",
            "--cc:clang",
            "--clang.exe=" & emcc.quoteShell(), "--clang.linkerexe=" &
                    emcc.quoteShell(), "-d:noSignalHandler"])
    ...
    if b.platform in ["android"]:
        b.nimFlags.add("-d:androidNDK")
    ...
    when defined(windows):
        b.nimFlags.add("-d:buildOnWindows") # Workaround for JS getEnv
    ...
    if b.platform in ["emscripten", "wasm", "js"]:
        ...
        if b.codesignIdentity.len != 0:
            b.signIosBundle()
            direShell "ios-deploy", "--debug", "--bundle", b.buildRoot /
                    b.bundleName, "--no-wifi"
...
proc runAutotestsInFirefox*(pathToMainHTML: string,
    ...
    """)
    let ffp = startProcess(ffbin, args = ["-profile", "./tempprofile",
            pathToMainHTML])
...
proc installAppOnConnectedDevice(b: Builder, devId: string) =
    ...
    var apkPath = b.buildRoot / b.javaPackageId / "build" / "outputs" / "apk" /
            conf / b.appName & "-" & conf & ".apk"
...
proc runAutotestsOnAndroidDevice*(b: Builder, devId: string,
        install: bool = true, extraArgs: StringTableRef = nil) =
    ...
    let logcat = startProcess(adb, args = ["-H", host, "-s", devId, "logcat",
            "-T", "1", "-s", "NIM_APP"])
    ...
    var args = @[adb, "-H", host, "-s", devId, "shell", "am", "start", "-S",
            "-n", activityName]
...
proc runAutotestsOnConnectedDevices*(b: Builder, install: bool = true,
        extraArgs: StringTableRef = nil) =
